...
docker cp $(docker ps -aqf "name=$CONTAINER_NAME"):C:/app/geckodriver.log "C:/git/port-scanner/scan-results/$TIMESTAMP"
...
docker cp $(docker ps -aqf "name=$CONTAINER_NAME"):/app/geckodriver.log "C:/git/port-scanner/scan-results/$TIMESTAMP"
fi